...
class Prediction(nn.Module):
    ...
    def forward(self, node_stacks, left_childs, encoder_outputs, num_pades, padding_hidden, seq_mask, mask_nums):
        ...
        # expanded_noises = torch.zeros((noises.size()[0], encoder_outputs.size()[0]))
        # augment = nn.Linear(1, encoder_outputs.size()[0]).cuda()
        # for i, st, noise in zip(range(noises.size()[0]), node_stacks, noises):
        #     if len(st) == 0:
        #         current_embeddings.append(padding_hidden)
        #         expanded_noise = augment(noise)
        #         expanded_noises[i] = expanded_noise
        #     else:
        #         current_node = st[-1]
        #         current_embeddings.append(current_node.embedding)
        #         expanded_noise = augment(noise)
        ...
        # expanded_noises = expanded_noises.unsqueeze(1).cuda()
        current_node_temp = []
